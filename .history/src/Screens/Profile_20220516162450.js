...
function Profile({ navigation }) {
  ...
  const handleSubmit = async () => {
    ...
    await setDoc(doc(citiesRef, 'users'), {
      Name: name,
      Email: email,
  ...
  const handleReadDB = async () => {
    ...
    querySnapshot.forEach((doc) => {
      //console.warn(`${doc.id} => ${JSON.stringify(doc.data(UID))}`);
    ...
    <View style={styles.root}>
      ...
      <Card style={styles.mycard} >
        <View style={styles.cardcontainer}>
          ...
          <Text style={{ marginTop: 12, fontSize: 15 }}> UserID: {userUID}</Text>
        </View>
      </Card>

      <View style={{ flexDirection: 'row', justifyContent: 'space-around', marginTop: 15, color: '#FF8C00' }}>
        <Button icon="account-edit" color="#FF8C00" mode="contained" onPress={handleReadDB}>
          Edit
        </Button>
      </View>
      <View style={{ flexDirection: 'row', justifyContent: 'space-around', marginTop: 15, color: '#FF8C00' }}>
